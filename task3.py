...
def boundaryRightFun(t, xRight):
    """This function calculates right boundary function
    """
...
numberOfSegm_x = 5  #numSeg - number of subsegments in segment [x_left, x_right]
numberOfSegm_t = 32  #numSeg - number of subsegments in segment [tInint, tFinal]
errorNorm = 0
epsNewton = 10**(-5)
...
for j in range(numberOfSegm_t):
    ...
    while flag:
        ...
        for i in range(0, numberOfSegm_x ):
            Fi = -(
            nonLinFun(yk[i]) - nonLinFun(uSolution[j][i]) - nonLinFunDer(yk[i]) * yk[i] - tStep * heterFun(timeGrid[j],spaceGrid[i])
